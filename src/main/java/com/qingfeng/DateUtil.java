...
public class DateUtil {
    ...
    /**
     * <p>将字日期date根据日期格式format转换为字符串</p>
     * @param date				待转换的字符串
     * @param format			日期格式
     * @return					转换之后的字符串
     */
    public final static String dateToFormatString(Date date, String format) {
        SimpleDateFormat formatDate = new SimpleDateFormat(format);
        String str = formatDate.format(date);
        return str;
    }
    /**
     * <p>将字符串formatString根据日期格式format转换为日期</p>
     * @param formatString		待转换的日期字符串
     * @param format			日期格式
     * @return					转换之后的日期
     */
    public final static Date formatStringToDate(String formatString, String format) {
        SimpleDateFormat formatDate = new SimpleDateFormat(format);
        Date date;
        try {
            date = formatDate.parse(formatString);
        } catch (ParseException e) {
            date = null;
            Assert.notNull(date, "参数formatString格式与format不匹配");
        }
        return date;
    }
    ...
    /**
     * 获取时间区间（给统计用）
     * @param tjfs 区间类型（日：day;周：week;月，month;季度quarter;年year;）
     * @param sj 统计时间（日:2020-09-19； 周：2020-09-21至2020-09-27； 月：2020-09， 季度：2020年，第2季度 年：2020）
     * */

    public static String gettjqj(String tjfs,String sj){
        ...
        try {
            switch (tjfs) {
                case "day":
                    startDay = sj + " 00:00:00";
                    endDay = sj + " 23:59:59";
                    break;
                case "week":
                    String[] str = sj.split("至");
                    startDay = str[0] + " 00:00:00";
                    endDay = str[1] + " 23:59:59";
                    break;
                case "month":
                    startDay = sj + "-01 00:00:00";
                    endDay = format.format(DateUtil.endOfMonth(format.parse(startDay)));
                    break;
                case "quarter":
                    switch (sj.substring(7, 8)) {
                        case "1":
                            startDay = sj.substring(0, 4) + "-01-01 00:00:00";
                            endDay = sj.substring(0, 4) + "-03-31 23:59:59";
                            break;
                        case "2":
                            startDay = sj.substring(0, 4) + "-04-01 00:00:00";
                            endDay = sj.substring(0, 4) + "-06-30 23:59:59";
                            break;
                        case "3":
                            startDay = sj.substring(0, 4) + "-07-01 00:00:00";
                            endDay = sj.substring(0, 4) + "-09-30 23:59:59";
                            break;
                        case "4":
                            startDay = sj.substring(0, 4) + "-10-01 00:00:00";
                            endDay = sj.substring(0, 4) + "-12-31 23:59:59";
                            break;
                        default:
                            System.out.println("无第" + sj.substring(6, 7) + "季度");
                    }
                    break;
                case "year":
                    startDay = sj + "-01-01 00:00:00";
                    endDay = format.format(DateUtil.endOfYear(format.parse(startDay)));
                    break;
                default:
                    System.out.println("统计方式格式错误");
            }
        } catch (Exception e) {
